...
def decorate_dashboard_html(html):
    ...
    html = html.replace('</style></head><body><main>', css + '</style></head><body><div class="dashboard-shell">' + navigation + '<main>')
...
function initHeadroomSorting(){'''
    ...
    dashboard_kpis_js = '''function dashboardKpis(){const banks=D.banks||[],groups=D.groups||[],years=Object.values(D.trend_series||{}).flatMap(series=>Object.values(series).flatMap(values=>Object.keys(values).map(Number))).filter(Number.isFinite),latest=years.length?Math.max(...years):"—",quality=D.in011?.in025?.trace_completeness||{};document.querySelector("#dashboard-kpis").innerHTML=[["Entities",banks.length,"regulated bank entities"],["Comparable cluster",banks.filter(b=>!b.insufficient).length,"with enough data for clustering"],["Parent groups",groups.length,"with reviewed context"],["Latest annual data",latest,"latest year in the series"]].map(k=>`<article class="kpi-card"><span class="kpi-label">${esc(k[0])}</span><strong class="kpi-value">${esc(k[1])}</strong><span class="kpi-note">${esc(k[2])}</span></article>`).join("");const badge=document.createElement("span");badge.className="freshness-badge";badge.textContent=`Trace complete: ${quality.complete_rows||0}/${quality.rows||0}`;document.querySelector("#dashboard-kpis").append(badge)}'''
    ...
    return html.replace('clusterBars();peerGroups();renderMetricSelectors();trendBars();groups();', 'dashboardKpis();dashboardSparklines();dashboardWatchlist();clusterBars();peerGroups();renderMetricSelectors();trendBars();groups();')
