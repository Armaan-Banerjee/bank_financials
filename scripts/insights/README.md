...
| `extract_metrics.py` | `banks/*.xlsx`, `data/reference/Banks List 2608.xlsx` | `insights.db` (banks, annual_metrics), `bank_metrics.csv` (+ `.schema.json`) | Parses every workbook sheet, matches each bank to its FRN (handling real sibling-entity ambiguity, e.g. Barclays Bank PLC vs Barclays Bank UK PLC), deduplicates on `(frn, sheet, row_label, year)`. |
