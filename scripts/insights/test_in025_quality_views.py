...
class QualityViewTests(unittest.TestCase):
    ...
    def test_quality_views_expose_searchable_trace_and_pdf_summary(self):
        ...
        self.assertEqual(html.count('class="appendix-panel"'), 2)
        self.assertNotRegex(html, r'<details class="appendix-panel"[^>]*\bopen\b')
        self.assertIn("open for bank/year detail", html)
        self.assertIn("open for row-level provenance", html)
