"""PROTOTYPE helper for IN-042 — throwaway, do not ship.

Renders a single bank's real .xlsx workbook (from banks/) as a self-contained,
tabbed static HTML page - one tab per sheet, laid out as a plain HTML table
with merged cells, bold rows, and header fills carried over from the
workbook's own openpyxl styling. No JS spreadsheet library, no server: this
is meant to be embedded via <iframe> in a bank-<slug>.html drilldown page
(imported and called from build_in042_prototype.py), or opened standalone.
