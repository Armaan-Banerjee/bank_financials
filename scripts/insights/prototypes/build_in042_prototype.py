"""PROTOTYPE generator for IN-042 — throwaway, do not ship.

Regenerates the static multi-page IN-042 mockup (comparison.html,
banks.html, bank-<slug>.html) from real data: research/in040_risk_metrics.json
for the 4 stress-test banks, plus a direct research/insights.db pull for
Weatherbys' Pillar 3 credit-risk-exposure-by-class breakdown (Weatherbys
discloses no IFRS 9 stage split at all, per IN-039's finding, so IN-040's
stage-only extraction has nothing to show for it - see IN-042's ticket
Resolution for why this substitute view was chosen over dropping the bank).

Run: python3 scripts/insights/prototypes/build_in042_prototype.py
Then open scripts/insights/prototypes/comparison.html directly in a browser.
"""
...
RISK_JSON = ROOT / "research" / "in040_risk_metrics.json"
SPEND_JSON = ROOT / "research" / "in041_spend_metrics.json"
...
def curate():
    d = json.loads(RISK_JSON.read_text())
    out = {}
    ...
    spend = json.loads(SPEND_JSON.read_text())
    for label, frn in BANKS.items():
        cap_years = {}
...
<link rel="stylesheet" href="in042_shared.css">
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.4/dist/chart.umd.min.js"></script>
</head>
<body>
<div class="proto-banner">PROTOTYPE v3 — IN-042 · separate static pages, not an SPA · real data, throwaway, not wired into the real dashboard</div>
...
<aside class="sidebar">
  ...
  <div class="sidebar-note">This prototype only has real data for 4 banks. At 145 banks, search still returns instantly and the sidebar never grows past this size — that's the point of moving the full list to its own page.</div>
  <div class="sidebar-foot">IN-042 · scripts/insights/prototypes/</div>
</aside>
...
def write_comparison(data, banks_index):
    html = HEAD.format(
        title="Credit risk — comparison (IN-042 prototype)",
        back_link="",
        h1="Credit risk",
        sub_fallback="",
        body="",
...
<script src="in042_shared.js"></script>
<script>
...
def write_banks_directory(data, banks_index):
    ...
    html = HEAD.format(
        title="Credit risk — banks (IN-042 prototype)",
...
def write_bank_page(bank_name, bank_data, banks_index):
    ...
    html = HEAD.format(
        title=f"{bank_name} — credit risk (IN-042 prototype)",
